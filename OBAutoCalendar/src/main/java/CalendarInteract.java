...
public class CalendarInteract
{
   ...
   */
   ...
   public static void borrarDuplicados(String calendario) throws Exception {
      com.google.api.services.calendar.Calendar service = getCalendarService();
      String calId = getIdCalendar(calendario, service);
      List<Event> lEventsAgendados = descargaEventsCalendar(calId);
      List<Event> lEventsUnicos = new ArrayList<>();
      List<Event> lEventsBorrar = new ArrayList<>();
      boolean esta = false;
      for(Event ev : lEventsAgendados)
      {
         for(Event evU : lEventsUnicos)
         {
            if(evU.getSummary().matches(ev.getSummary()))
               esta=true;
            else{
               esta=true;
               break;
         ...
         if(!esta)
         {
            lEventsUnicos.add(ev);
            esta=false;
         }
         else
            lEventsBorrar.add(ev);
      }
      for(Event evBorrar : lEventsBorrar)
         borraEventCalendar(evBorrar.getId(), calId);
   }

   public static void subirEvents(List<CPUEvent> listaSubir, String calendario) throws Exception {
      com.google.api.services.calendar.Calendar service = getCalendarService();
      String calId = getIdCalendar(calendario, service);
      // Descargar los eventos agendados
      List<Event> lEventsAgendados = descargaEventsCalendar(calId);
      List<CPUEvent> lEventsTotales = listaSubir;
      List<CPUEvent> lEventsOrdenados;
      for(Event newArgG : lEventsAgendados)
      {
         if(listaSubir.size() > 0 && newArgG.getSummary()!=null && newArgG.getSummary().matches("^"+listaSubir.get(0).tipo+".*$")) {
            CPUEvent argG = new CPUEvent(newArgG);
      ...
      {
         for(CPUEvent evSubir : lEventsOrdenados)
         {
            if(evSubir != null && evAg.getLocation()!=null && evAg.getLocation().matches(evSubir.getEvent().getLocation()))
            {
               if(evAg.getId() != null)
      ...
      List<CPUEvent> lEventsParaSubir = compruebaEventosAgendados(lEventsOrdenados, calendario);
      for(CPUEvent ArgEv : lEventsParaSubir) {
         if (ArgEv != null) {
...
*/
   ...
   public static List<CPUEvent> compruebaEventosAgendados(List<CPUEvent> lEventsOrdenados, String calendario) throws Exception {
      DateTime now = new DateTime(System.currentTimeMillis());
      //int evArgTipoNotif;
      com.google.api.services.calendar.Calendar service = getCalendarService();
      //CPUEvent arg = new CPUEvent(lEventsOrdenados.get(0).getEvent());
      String calId = getIdCalendar(calendario, service);
      List<CPUEvent> lEventsParaSubir = new ArrayList<CPUEvent>();
      boolean added = false;
      ...
      if (lEventsOrdenados.size() > 0) {
         for (CPUEvent argOrd : lEventsOrdenados) {
            if (lEventsParaSubir.size() == 0)
               lEventsParaSubir.add(argOrd);
            else {
               for (CPUEvent argSub : lEventsParaSubir) {
                  if (argSub != null && argOrd.getEvent().getLocation().matches(argSub.getEvent().getLocation()))
                     added = true;
               }
               if (!added) {
                  lEventsParaSubir.add(argOrd);
         ...
         for (CPUEvent argPaSubir : lEventsParaSubir) {
            for (CPUEvent argOrd : lEventsOrdenados) {
               if (argOrd != null && argPaSubir.getEvent().getLocation().matches(argOrd.getEvent().getLocation())) {
...
*/
   private static CPUEvent masReciente(List<CPUEvent> lEventos) {
      CPUEvent masReciente = null;
      if (lEventos.size() > 0) {
         masReciente = lEventos.get(0);
         for (CPUEvent ev : lEventos) {
            if (ev.getFechaRecibido() != null && masReciente.getFechaRecibido() != null)
            {
               if (ev.getFechaRecibido().getValue() > masReciente.getFechaRecibido().getValue())
                  masReciente = ev;
            }
            else
               System.out.println("Fecha recibido nula en:\r" + ev.event.getSummary());
         }
         return masReciente;
      }
      else
...
*/
   public static List<CPUEvent> ordenarEvents(List<CPUEvent> lEventsParaOrdenar)
   {
      List<CPUEvent> lEventsDevolver = new ArrayList<CPUEvent>();
      List<CPUEvent> lEventsOrd = new ArrayList<CPUEvent>();
      CPUEvent ar = null;
      int i = lEventsParaOrdenar.size();
      do {
         ar = masReciente(lEventsParaOrdenar);
      ...
      boolean esta = false;
      if(lEventsOrd.size()>0) {
         for(CPUEvent evOrd : lEventsOrd) {
            if(lEventsDevolver.size()==0)
               lEventsDevolver.add(lEventsOrd.get(0));
            for(CPUEvent evDev : lEventsDevolver) {
               if(evDev != null && evDev.getEvent().getLocation().matches(evOrd.getEvent().getLocation())) {
                  esta = true;
               }
            }
            if(!esta)
